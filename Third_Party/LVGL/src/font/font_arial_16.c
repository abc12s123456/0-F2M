...
#include "ff.h"
static uint8_t __g_font_buf[153];//如bin文件存在SPI FLASH可使用此buff

static uint8_t font_is_open = 0;
static FIL     font_fs;
...
static uint8_t *__user_font_getdata(int offset, int size)
{
    ...
    if(font_is_open == 0)
    {
        res = f_open(&font_fs, "0:/font/font_arial_16.bin", FA_READ);
        if(res)
        {
            return NULL;
        }
    ...
    f_lseek(&font_fs, offset);
    
    res = f_read(&font_fs, __g_font_buf, size, &num);
    if(res || num == 0)
    {
        return NULL;
    }
